...
export default function FutureVaccine({ route }) {
...
console.log('EmailFutureVaccine: ',itemId);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${endpoint}/GetFutureVaccinesForParent/?Parent_Email=${itemId}`);
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.card1}>
                ...
                <Text>Health Care Worker: {item.HCW_Email}</Text>
              </View>
            )}
          />
        )}
